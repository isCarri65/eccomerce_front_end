...
export const AdminProducts: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editMode && form.id) {
      setProducts((products) =>
        products.map((p) =>
          p.id === form.id ? ({ ...form, id: form.id } as IProduct) : p
        )
      );
    } else {
      ...
      setProducts((products) => [
        ...products,
        { ...form, id: newId, categories: form.categories || [] } as IProduct,
      ]);
    }
    setShowForm(false);
  };
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (filesImages) {
      setForm((prev: any) => ({ ...prev, images: Array.from(filesImages) }));
    }
  };
  ...
  const handleSizeToggle = (id: number) => {
    setForm((prev: any) => {
      ...
      return {
        ...prev,
        sizes: exists
          ? prev.sizes.filter((sid: number) => sid !== id)
          : [...(prev.sizes || []), id],
      };
    });
  };
        ...
        <>
          ...
          <div className={styles.tableWrapper}>
            <table
              ...
              style={{ width: "70vw", minWidth: 600, maxWidth: 1200 }}
            >
              <thead>
                <tr>
                  <th>Producto</th>
          ...
          <div className={styles.formGrid}>
            <div className={styles.leftCol}>
              ...
              <div className={styles.formRow}>
                ...
                <select
                  name="state"
                  value={form.state ? "1" : "0"}
                  onChange={(e) =>
                    setForm((f: any) => ({
                      ...f,
                      state: e.target.value === "1",
                    }))
                  }
                >
